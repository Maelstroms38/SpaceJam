...
@property (nonatomic) SKAction *laserSFK;
...
-(void) setupSounds {
    ...
    self.explodeSFX = [SKAction playSoundFileNamed:@"explosion.caf" waitForCompletion:NO];
    self.laserSFK = [SKAction playSoundFileNamed:@"Shoot.caf" waitForCompletion:NO];
    
    NSURL *url = [[NSBundle mainBundle] URLForResource:@"Mantis" withExtension:@"mp3"];
...
- (void) update:(NSTimeInterval)currentTime {
    NSLog(@"%f",fmod(currentTime, 60)); //time loop
...
-(void)didBeginContact:(SKPhysicsContact *)contact {
    ...
    if ( contact.bodyA.categoryBitMask == THCollisionCategoryEnemy && contact.bodyB.categoryBitMask == THCollisionCategoryProjectile ) {
...
-(void)createDebrisAtPosition:(CGPoint)position {
    NSInteger numberOfPieces = [THUtil randomWithMin:2 max:5];
    ...
    for (int i=0; i < numberOfPieces; i++) {
        ...
        [debris runAction:[SKAction waitForDuration:1.0] completion:^{
    ...
    [explosion runAction:[SKAction waitForDuration:1.0] completion:^{[explosion removeFromParent];
     }];
}
@end
